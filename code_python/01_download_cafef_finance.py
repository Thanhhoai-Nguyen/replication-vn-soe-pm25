# pipeline_vn_data_1week/code_python/01_download_cafef_finance.py
...
TIMEOUT = 30
SLEEP_RANGE = (0.8, 1.6)
TOTALROW = 80  # keep enough then FILTER to 2020-2022
REPORT_TYPE = "QUY"  # quarterly
SORT = "DESC"        # newest first
TYPE = 1             # same as your current usage
...
def main() -> None:
    print("SCRIPT START: download cafef finance JSON (QUY) and keep 2020-2022 only")

    ROOT = Path(__file__).resolve().parents[1]
    INP = ROOT / "0_input" / "sample_500.csv"
    OUTDIR = ROOT / "1_raw" / "cafef_finance"
    ...
    for i, sym in enumerate(tickers, 1):
        sym_l = sym.lower()
        out_path = OUTDIR / f"{sym}_type1_QUY_{START_YEAR}_{END_YEAR}.json"
        ...
        try:
            ...
            # parse JSON and FILTER to 2020-2022
            payload = r.json()

            # expected structure: payload["Data"]["Value"] = list of quarters
            ...
            if not isinstance(values, list):
                ...
                debug_path = OUTDIR / f"{sym}_BAD_RESPONSE.txt"
                debug_path.write_text(r.text, encoding="utf-8")
                raise RuntimeError("Unexpected JSON structure. Saved BAD_RESPONSE.txt")

            kept = [item for item in values if in_year_range(item.get("Time"))]

            # Replace list with filtered
            data["Value"] = kept
